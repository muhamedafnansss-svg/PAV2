"""
VAL API Server v15.0 — JARVIS Operator Console
=================================================
Security-hardened. CORS locked to localhost. Rate-limited.
SAFE/POWER/LAB security modes. Voice-first JARVIS interface.
"""
...
VAL_SYSTEM_PROMPT = (
    "You are VAL, an elite AI executive assistant operating as a local AI OS. "
    "You speak with calm intelligence, like JARVIS — precise, capable, never verbose. "
    "Execute commands, analyze threats, write code, and control the system. "
    "Be concise, technical, and actionable. Use markdown for code. "
    "Never apologize or explain what you can't do — focus on what you can."
)
...
if _OK:
    ...
    def build_app() -> "FastAPI":
        ...
        async def lifespan(app):
            logger.info("[VAL] Server v15.0 starting — JARVIS Operator Console")
        ...
        async def voice_status():
            try:
                ...
                return {"stt": {"available": False}, "tts": {"available": False}, "error": str(e)}
        ...
        async def voice_transcribe(request: Request):
            ...
            text = await loop.run_in_executor(
                _tools_executor,
                lambda: bridge.transcribe_bytes(audio_data, filename)
            )
        ...
        async def voice_speak_endpoint(req: VoiceSpeakRequest):
            from val.voice.voice_bridge import get_voice_bridge
            bridge = get_voice_bridge()
            bridge.speak(req.text, async_mode=True)
            return {"status": "speaking", "tts_available": bridge.tts.available}
        ...
        async def voice_mode(request: Request):
            body = await request.json()
            mode = body.get("mode", "formal")
            from val.voice.voice_bridge import get_voice_bridge
            bridge = get_voice_bridge()
            bridge.set_voice_mode(mode)
            return {"voice_mode": mode}

        @app.post("/voice/enroll")
        async def voice_enroll(request: Request):
            """Enroll owner voice. Expects multipart with 2+ audio files."""
            import numpy as np
            form = await request.form()
            samples = []
            for key in form:
                ...
                if hasattr(f, 'read'):
                    ...
                    try:
                        ...
                        import io as _io
                        audio, sr = sf.read(_io.BytesIO(data))
                        samples.append(audio.astype(np.float32))
                    except Exception:
                        pass
            if len(samples) < 2:
                raise HTTPException(400, "Need at least 2 voice samples")
            ...
            success = await loop.run_in_executor(
                _tools_executor,
                lambda: bridge.auth.enroll(samples)
            )
            return {"enrolled": success, "samples": len(samples)}

        @app.get("/voice/auth/status")
        async def voice_auth_status():
            from val.voice.voice_bridge import get_voice_bridge
            bridge = get_voice_bridge()
            return bridge.auth.status()
        ...
        async def system_control(request: Request):
            ...
            loop = asyncio.get_running_loop()
            from val.tools.system_control import execute_system_command
            result = await loop.run_in_executor(
                _tools_executor,
                lambda: execute_system_command(command)
            )
...
def start_api_server(host: str = "127.0.0.1", port: int = 8765) -> None:
    if not _OK:
        ...
        return
    app = build_app()
    print(f"\n[VAL] API v15.0 - JARVIS Operator Console")
