...
=============================
Centralized, validated configuration for the VAL AI system.
...
def build_config() -> AppConfig:
    ...
    gemma_cfg = ModelConfig(
        name="gemma",
        model_type="gemma",
        model_path=MODELS_ROOT / "gemma",
        max_new_tokens=int(os.getenv("GEMMA_MAX_TOKENS", "512")),
        temperature=float(os.getenv("GEMMA_TEMPERATURE", "0.7")),
        top_p=float(os.getenv("GEMMA_TOP_P", "0.9")),
        max_context_length=int(os.getenv("GEMMA_CONTEXT_LEN", "1500")),  # Capped
        device=device,
        load_in_4bit=_bool_env("GEMMA_4BIT", True),      # 4-bit ON by default
        load_in_8bit=_bool_env("GEMMA_8BIT", False),
        enabled=_bool_env("GEMMA_ENABLED", True),
    )
